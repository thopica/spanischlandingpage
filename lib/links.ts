export const PHONE_DISPLAY = "+41 77 292 09 03";
export const PHONE_TEL = "tel:+41772920903";

/** Shown on the seniors landing contact card (optional copy). */
export const PHONE_HOURS =
  "Werktags von 9 bis 18 Uhr, am liebsten zwischen 10 und 12.";

const WHATSAPP_DIGITS = "41789031057";
const WHATSAPP_PRESET =
  "Hola Cristina! Ich interessiere mich für eine gratis Probelektion.";

export const WHATSAPP_URL = `https://wa.me/${WHATSAPP_DIGITS}?text=${encodeURIComponent(
  WHATSAPP_PRESET,
)}`;

export const EMAIL = "mcristinacaamano@gmail.com";
export const EMAIL_MAILTO = `mailto:${EMAIL}?subject=${encodeURIComponent(
  "Anfrage Spanischunterricht",
)}`;

export const CITY = "Zürich";
export const SITE_NAME = "Spanisch mit Cristina";
